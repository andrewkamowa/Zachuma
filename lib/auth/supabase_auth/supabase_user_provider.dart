...
class ZachumaSupabaseUser extends BaseAuthUser {
  ZachumaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> zachumaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ZachumaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
